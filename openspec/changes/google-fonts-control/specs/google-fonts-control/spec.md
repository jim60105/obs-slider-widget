...
The page SHALL load "Yusei Magic" from Google Fonts via a `<link>` element in `<head>` using the URL `https://fonts.googleapis.com/css2?family=Yusei+Magic&display=swap`.

#### Scenario: Default font link in head
- **WHEN** the page HTML is inspected
- **THEN** a `<link rel="stylesheet">` element with the Google Fonts URL for "Yusei Magic" with `display=swap` SHALL be present in `<head>`
...
- **WHEN** "Yusei Magic" is loaded and applied
- **THEN** `document.documentElement.style.fontFamily` SHALL include "Yusei Magic" followed by the fallback system font stack
