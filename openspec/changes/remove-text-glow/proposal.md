...
- **Code**: `style.css` (remove 2 glow layers from `.progress-title` and `.progress-status` text-shadow rules), `app.js` (remove any inline text-shadow glow logic for `#displayPercent`)
- **Specs**: `text-glow` spec removed entirely; `text-white-outline` spec updated to remove glow coexistence references
- **Dependencies**: None
