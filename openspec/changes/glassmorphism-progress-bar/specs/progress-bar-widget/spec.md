...
The display area SHALL contain a glass track element with capsule shape (`width: 20px`, `height: 400px`, `border-radius: 999px`). The track SHALL have a glassmorphism effect: semi-transparent background, `backdrop-filter: blur(12px)`, a semi-transparent border, and a subtle box-shadow. The track SHALL use `overflow: hidden` and `position: relative`.

#### Scenario: Track renders with correct dimensions
- **WHEN** the page loads
- **THEN** the glass track element SHALL be visible with width 20px, height 400px, and fully rounded corners (border-radius 999px)
...
The progress bar widget container SHALL use a flex row layout with a gap between elements. Items SHALL be aligned to the bottom (flex-end).

#### Scenario: Elements are laid out horizontally
- **WHEN** the page loads
- **THEN** the title text, glass track, and percentage display SHALL be arranged in a horizontal row with spacing, aligned to the bottom
