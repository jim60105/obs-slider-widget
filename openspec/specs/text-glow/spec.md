...
The `.progress-title` element SHALL have a multi-layered `text-shadow` that includes both white outline layers and a visible glow effect using `var(--theme-color)`.
...
- **THEN** the `.progress-title` glow color SHALL update in real time to match the new `--theme-color` value while the white outline layers remain unchanged

### Requirement: Progress status glow
The `.progress-status` element SHALL have a multi-layered `text-shadow` that includes both white outline layers and a visible glow effect using `var(--theme-color)`.
...
- **WHEN** the user changes the theme color via the color picker
- **THEN** the `.progress-status` glow color SHALL update in real time to match the new `--theme-color` value while the white outline layers remain unchanged

### Requirement: Display percent glow
The `#displayPercent` element SHALL have a `text-shadow` glow effect applied via its inline style, using the current theme color, combined with white outline layers.
...
- **WHEN** the user changes the theme color via the color picker
- **THEN** the `#displayPercent` glow SHALL update to use the new theme color value while the white outline layers remain unchanged
