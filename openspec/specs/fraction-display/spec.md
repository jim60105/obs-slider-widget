...
The widget SHALL display progress as a fraction in the format "current/total" (e.g., "12/20") instead of a percentage format (e.g., "60%"). The fraction text SHALL render in vertical writing mode, with font size controlled via inline `style.fontSize` rather than a Tailwind utility class.
