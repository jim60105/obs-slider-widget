...
A custom dropdown component for OBS browser source compatibility. Native `<select>` elements do not render their option popups correctly in OBS (CEF-based Chromium), so this custom dropdown provides equivalent functionality using standard DOM elements that work reliably in OBS browser sources.
